...
import requests
from evaluate_strategy import evaluate_performance  # 引入自定义评估函数
...
# 设置 FMP API 密钥
api_key = 'qhylk6wN8OUWRPCo59NmBU'
...
# 📌 9. 策略回测与评估
# ==================================================
# 假设 `strategy.equity_curve` 为策略的资金曲线，实际的股价是 train_data['Close']
# 这时需要传入实际股价数据的收盘价（train_data['Close']）
# 你可以替换成适合你策略的实际股价数据

# 假设 trade_history 为模拟的交易历史（根据策略的买卖生成）
trade_history = []  # 这里你需要替换成实际的交易历史

# 调用评估函数，评估策略表现
performance = evaluate_performance(np.array(simulated_prices[1:]), train_data['Close'].values, trade_history)
